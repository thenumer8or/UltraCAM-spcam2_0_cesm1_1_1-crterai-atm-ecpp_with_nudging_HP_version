...
!!!   call check_energy_chng(state, tend, "drymass", nstep, ztodt, zero, zero, zero, zero)
    ...
    !===================================================                              
! HP moved the following calls to the end of tphysbc.
...
! as it was added above as part of crm tendency.
    ...
    ! Call nudging at the end of tphysbc
    ! HP moved the nudging call here after the discovery that the nudging call inside tphysac practically ignores the 
    ! sensible heat flux leading to a negative definite Nudge_T near the surface (ref: Chris T. email dated March 19, 2019). 
    ...
    if ((Nudge_Model .and. Nudge_ON)) then
     ...
     if (masterproc) then
      write(iulog,*) 'Updated nudging values at the end of tphysbc'
     endif
